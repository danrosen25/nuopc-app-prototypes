...
module COMPO
  ...
  subroutine Advance(model, rc)
    ...
    call ESMF_VMLog(vm, prefix="COMPO Advance(): ", &
      logMsgFlag=ESMF_LOGMSG_INFO, rc=rc)
    RC_CHECK(rc)

    call ESMF_ClockPrint(clock, options="currTime", &
      preString="------>Advancing COMPO from: ", unit=msgString, rc=rc)
    RC_CHECK(rc)
    ...
    call NUOPC_ModelTagPrint(model, rc=rc)
    RC_CHECK(rc)
...
!    ! print info from importState
    ...
    call ESMF_InfoPrint(info, &
      preString="COMPO_INFO:"//trim(currTimeString), rc=rc)
    RC_CHECK(rc)
...
subroutine SetVM(comp, rc)
  use ESMF
  use COMPO, only: SetVMModule => SetVM
...
subroutine SetServices(comp, rc)
  use ESMF
  use COMPO, only: SetServicesModule => SetServices
  type(ESMF_GridComp) :: comp
  integer, intent(out) :: rc
