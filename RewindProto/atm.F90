...
#define WITHIMPORTFIELDS
  ...
  subroutine SetClock(model, rc)
    ...
    call ESMF_ClockGet(modelClock, startTime=prevCurrTime, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
  ...
  subroutine CheckImport(model, rc)
    ...
    if (timeCheck) then
      ...
      if (.not.timeCheck) then
        call ESMF_LogWrite("ATM: NUOPC INCOMPATIBILITY DETECTED: "// &
  ...
  subroutine Advance(model, rc)
    ...
    if (currTime .lt. prevCurrTime) then
      msgString="ATM: Clock rewind caught"
    ...
    call ESMF_ClockPrint(modelClock, options="currTime", &
      preString="ATM: current time = ", unit=msgString, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    call ESMF_ClockGet(modelClock, currTime=prevCurrTime, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
